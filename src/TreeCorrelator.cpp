...
void TreeCorrelator::addChild(string parent, string child, bool coin /* = true*/, bool verbose /*= false*/) {
	if (places_.count(parent) == 1 || places_.count(child) == 1) {
		place(parent)->addChild(place(child), coin);
		if (verbose) { cout << "TreeCorrelator: setting " << child << " as a child of " << parent << endl; }
	} 
	else {
		stringstream ss;
	...
	if (name_tokens.size() > 1) {
		...
		if (name_tokens.back().find("-") != string::npos || name_tokens.back().find(",") != string::npos) {
			...
			for (vector<string>::iterator itc = comma_token.begin(); itc != comma_token.end(); ++itc) {
				vector<string> range_tokens = strings::tokenize((*itc), "-");
				int range_min = strings::to_int(range_tokens[0]);
				int range_max = range_min;
				if (range_tokens.size() > 1){ range_max = strings::to_int(range_tokens[1]); }
		...
		else { names.push_back(base_name + name_tokens.back()); } 
	} 
	else { names.push_back(name); }
	return names;
}

void TreeCorrelator::createPlace(map<string, string>& params, bool verbose /*= false*/) {
	bool replace = false;
	if (params["replace"] != ""){ replace = strings::to_bool(params["replace"]); }
	...
	for (vector<string>::iterator it = names.begin(); it != names.end(); ++it) {
		if (params["type"] != "") {
			if (replace) {
				if (places_.count((*it)) != 1) {
					stringstream ss;
					ss << "TreeCorrelator: cannot replace Place" << (*it) << ", it doesn't exist";
					throw TreeCorrelatorException(ss.str());
				}
				delete places_[(*it)];
				if (verbose::CORRELATOR_INIT){ cout << "TreeCorrelator: replacing place " << (*it) << endl; }
			} else {
				if (places_.count((*it)) == 1) {
					stringstream ss;
				...
				if (verbose::CORRELATOR_INIT){ cout << "TreeCorrelator: creating place " << (*it) << endl; }
			}
		...
		if (params["parent"] != "root") {
			...
			addChild(params["parent"], (*it), coincidence, verbose::CORRELATOR_INIT);
		}
	}
}
...
void TreeCorrelator::buildTree() {
	...
	string xmlFileName = "./setup/TreeCorrelator.xml";
	pugi::xml_parse_result result = doc.load_file(xmlFileName.c_str());
	cout << "Loading configiration file " << xmlFileName << " : " << result.description() << endl;
	cout << "Configuration description: " << doc.child("TreeCorrelator").attribute("description").value() << endl;
...
TreeCorrelator::~TreeCorrelator() {
	for (map<string, Place*>::iterator it = places_.begin(); it != places_.end(); ++it) {
		if (verbose::MAP_INIT){ cout << "TreeCorrelator: deleting place " << (*it).first << endl; }
		delete it->second;
	}
}
