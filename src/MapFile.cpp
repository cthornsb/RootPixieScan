...
{  
	...
	while (!in.eof()) {
		...
		else { normalLines.push_back(tokenList); }
	}

	// now we parse each tokenized line, processing wildcard lines afterwards
	for (vector< vector<string> >::iterator it = normalLines.begin(); it != normalLines.end(); it++) {
		ProcessTokenList(*it);
	}
	for (vector< vector<string> >::iterator it = wildcardLines.begin(); it != wildcardLines.end(); it++) {
		ProcessTokenList(*it);
	}
	...
	for (map<string, Place*>::iterator it = TreeCorrelator::get()->places.begin(); it != TreeCorrelator::get()->places.end(); ++it) {
		...
		cout << "No. of children " << (*it).second->children_.size() << endl;
		for (unsigned i = 0;
...
void MapFile::ProcessTokenList(const vector<string> &tokenList) const
{
	...
	if (tokenIt != tokenList.end() && tokenIt->find_first_not_of("1234567890") != string::npos) {
		// synonym for "Same as type"
		if ( *tokenIt == "--" ) { subtype = type; } 
		else { subtype = *tokenIt; }
		tokenIt++;
	} 
	else { subtype = type; }
	...
	else { startingLocation = modChan->GetNextLocation(type, subtype); }
	...
	for (vector<int>::iterator modIt = moduleList.begin(); modIt != moduleList.end(); modIt++) {
		for (vector<int>::iterator chanIt = channelList.begin(); chanIt != channelList.end(); chanIt++) {
