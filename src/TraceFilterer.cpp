 ...
 */
TraceFilterer::PulseInfo::PulseInfo(Trace::size_type theTime, double theEnergy) : time(theTime), energy(theEnergy)
{
	isFound = true;
}
...
TraceFilterer::TraceFilterer(int offset, int range) : TracePlotter(offset, range), fastParms(5,5), energyParms(100,100), thirdParms(20,10)
{
	//? this uses some legacy values for third parms, are they appropriate
	name = "Filterer";
...
bool TraceFilterer::Init(const string &filterFileName /* = filter.txt */)
{
	...
	ifstream in(("./setup/"+filterFileName).c_str());
	if (!in) {
	...
	while (!in.eof()) {
		if ( isdigit(in.peek()) ) {
			...
			cout << "  Fast filter: " << rise << " " << gap << " " << fastThreshold << endl;
			...
			cout << "  Third filter: " << rise << " " << gap << " " << slowThreshold;
			if (!useThirdFilter) { cout << ", not used"; }
			cout << endl;
...
void TraceFilterer::Analyze(Trace &trace, const string &type, const string &subtype)
{
	...
	if (level >= 5) {
		...
		if ( trace.GetValue("sigmaBaseline") > deviationCut || abs(trailingBaseline - trace.GetValue("baseline")) < deviationCut) {		
...
const TraceFilterer::PulseInfo& TraceFilterer::FindPulse(Trace::iterator begin, Trace::iterator end)
{
	...
	static binder2nd< greater<Trace::value_type> > crossesThreshold (greater<Trace::value_type>(), fastThreshold);
	...
	while (begin < end) {
		...
		if (useThirdFilter) {
			...
			if (sample >= thirdFilter.size() || thirdFilter[sample] < slowThreshold) {
				begin++; 
				continue;
			}
		}
		...
		if (sample < energyFilter.size()) {
			...
			if (presample >= 0) { pulse.energy -= energyFilter[presample]; }
